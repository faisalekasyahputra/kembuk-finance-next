...
function ReceiptExportView({ transactions, balance, totalIncome, totalExpense, onClose }: {
  ...
  const handleSendToTelegram = async () => {
    ...
    const topIncome = Object.entries(groupedByCategory)
...
${topIncome.length > 0 ? `📈 *SUMBER PEMASUKAN*

${topIncome.map(([name, data]) => `├ ${name}: Rp ${data.total.toLocaleString('id-ID')}`).join('\n')}

━━━━━━━━━━━━━━━━━` : ''}

${topExpenses.length > 0 ? `📉 *PENGELUARAN TERBESAR*

${topExpenses.map(([name, data]) => `├ ${name}: Rp ${data.total.toLocaleString('id-ID')}`).join('\n')}
          ...
          canvas.toBlob(async (blob) => {
            ...
            await fetch(`https://api.telegram.org/${botToken}/sendMessage`, {
              ...
              body: JSON.stringify({
                ...
                reply_markup: {
                  inline_keyboard: [[
                    { text: '📱 Buka App', url: 'https://kembuk-finance-next.vercel.app' }
                  ]]
                }
              })
            })
